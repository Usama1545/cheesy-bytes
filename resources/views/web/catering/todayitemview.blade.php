                ...
                <div class="d-flex justify-content-between align-items-center mb-1">
                 ...
                 <h5 class="fs-6 mb-0 item-card-title d-flex text-h">
                    <div class="d-flex gap-1">
                        @if($itemdata->deal_type == 1 || $itemdata->deal_type == 3)

                            <a class="cursor-pointer" data-bs-toggle="modal"
                                data-bs-target="#DealModal"
                                data-deal-id="{{ $itemdata->deal_id }}"
                                data-deal-title="{{ $itemdata->product->item_name }}">
                                @else
                                    <a class="cursor-pointer" onclick="showdealitem('{{ $itemdata->product->slug }}','{{ $itemdata->deal_id }}','{{ URL::to('/show-deal-item') }}')">
                    ...
                    <div class="d-sm-flex justify-content-between align-items-center">
                        ...
                        <div class="d-sm-flex gap-2 align-items-center mt-lg-0 mt-1">
                            ...
                            @else
                                @if($itemdata->deal_type == 1 || $itemdata->deal_type == 3)
                                    <button
                                        class="btn btn-sm btn-secondary w-100 fw-500 py-2 px-4 float-end rounded-3 d-flex gap-2 justify-content-center align-items-center addon_modal_{{ $itemdata->product->slug }}"
                                        data-bs-toggle="modal"
                                        data-bs-target="#DealModal"
                                        data-deal-id="{{ $itemdata->deal_id }}"
                                        data-deal-title="{{ $itemdata->product->item_name }}"
