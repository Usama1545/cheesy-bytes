    ...
    <div class="card rounded-4 overflow-hidden h-100">
        ...
        @if ($isPizza)
            <a data-bs-toggle="modal" data-bs-target="#PizzaModal" class="btn btn-sm fw-500 border-0 cursor-pointer"
                data-product-id="{{ $itemdata->id }}" data-deal-id="{{ $dealId }}"
        ...
        <div class="card-image">
            <img src="{{ @helper::image_path($itemdata['item_image']->image_name) }}"
                class="card-img-top border-0 rounded-0 rounded-top position-relative" alt="dishes" loading="lazy"
                decoding="async">
        </div>
        </a>
                ...
                @if ($itemdata->is_cart == 1)
                    <div class="item-quantity py-1 px-5">
                        ...
                        @if ($isPizza)
                            @if ($canAddMore)
                                <a data-bs-toggle="modal" data-bs-target="#PizzaModal"
                                    class="btn btn-sm fw-500 border-0" data-deal-id="{{ $dealId }}"
                                    data-product-id="{{ $itemdata->id }}" data-category-id="{{ $categoryId }}">+</a>
                            @else
                    ...
                    @if ($canAddMore)
                        @if ($isPizza)
                            <button
                                ...
                                data-bs-toggle="modal" data-bs-target="#PizzaModal" data-deal-id="{{ $dealId }}"
