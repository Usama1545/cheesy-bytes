...
@section('content')
    <div class="container">
        @foreach ($result as $deal)
            ...
            @if (($deal['category_meta']['is_free'] ?? 0) != 1)
                    ...
                    <div class="container">
                        <div class="row g-3 align-items-center justify-content-between mb-sm-5 mb-4">
                            <div class="col-auto menu-heading">
                                <p class="sub-lables text-capitalize mt-2 mb-0">
                                    {{ ($loop->first && !empty($deal['custom_message'])) ? $deal['custom_message'] : $deal['category_name'] }}
                                </p>

                                <h1 class="text-uppercase">
                                    Select {{ $deal['category_meta']['max'] }}
                                    {{ $deal['category_name'] }}
                                </h1>
                            </div>
                        </div>
                        ...
                        <div class="row g-4">
                            @foreach ($deal['items'] as $itemdata)
                                @include('web.home1.dealitemview')
                            @endforeach
                        </div>
                    </div>
                </section>
