    ...
    <div class="row align-items-center">
        ...
        <div class="item-details">
            @foreach ($itemdata['addons_group'] as $addons_group)
                @php
                    $availableAddons = $addons_group->availableAddons;
                @endphp
                @if ($availableAddons->isNotEmpty())
                    <div class="item-addons-list mt-3 border-bottom pb-3"
                        ...
                        @foreach ($addons_group->availableAddons as $addons)
                            <div class="{{ session()->get('direction') == '2' ? 'd-flex gap-2 me-2' : 'form-check' }}">
                                @php
                                    $type = $addons_group->selection_count == 1 ? 'radio' : 'checkbox';
                                @endphp
                                <input
                                    class="form-check-input cursor-pointer addon-checkbox addons_chk_{{ $itemdata['id'] }} {{ session()->get('direction') == '2' ? 'ms-0' : '' }}"
                                    type="{{ $type }}" value="{{ $addons->id }}"
                                    data-addons-id="{{ $addons->id }}" data-item-id="{{ $itemdata['id'] }}"
                                    data-addons-price="{{ $addons->price }}" data-addons-name="{{ $addons->name }}"
                                    onclick="toggleCrusts('{{ $addons->id }}', '{{ $itemdata['id'] }}'); getaddons('{{ $itemdata['id'] }}')"
                                    id="addons_{{ $addons->id }}_{{ $itemdata['id'] }}"
                            ...
                            @if ($addons->crusts && $addons->crusts->isNotEmpty())
                                <div id="crust-options-{{ $addons->id }}-{{ $itemdata['id'] }}"
                                    class="crust-options row ml-2 d-none" style="margin-left: 25px">
                                    @foreach ($addons->crusts as $index => $crust)
                                        <div class="form-check col-2">
                                            <input type="radio"
                                                class="form-check-input cursor-pointer addons_chk_crust_{{ $itemdata['id'] }}"
                                                name="crust_{{ $addons->id }}_{{ $itemdata['id'] }}"
                                                id="crust_{{ $crust->id }}_{{ $addons->id }}_{{ $itemdata['id'] }}"
                                                value="{{ $crust->id }}"
                                                data-crust-price="{{ $crust->crust_price }}"
                                                data-addons-id="{{ $addons->id }}"
                                                data-crust-name="{{ $crust->crust->name }}"
                                                {{ $loop->first ? 'checked' : '' }}> <!-- Auto-select first crust -->

                                            <label class="form-check-label cursor-pointer fs-7"
                                                for="crust_{{ $crust->id }}_{{ $addons->id }}_{{ $itemdata['id'] }}">
                                                {{ $crust->crust->name }}
                                            </label>
                                        </div>
                                    @endforeach
        ...
        <input type="hidden" name="deal_category_id" id="deal_category_id_{{ $itemdata['id'] }}"
            value="{{ $itemdata['deal_category_id'] ?? '' }}">
        <input type="hidden" name="item_name" id="item_name_{{ $itemdata['id'] }}"
            value="{{ $itemdata['item_name'] }}">
        <input type="hidden" name="item_type" id="item_type_{{ $itemdata['id'] }}"
            value="{{ $itemdata['item_type'] }}">
        <input type="hidden" name="image_name" id="image_name_{{ $itemdata['id'] }}"
            value="{{ $itemdata['image_name'] }}">
        <input type="hidden" name="tax" id="item_tax_{{ $itemdata['id'] }}" value="{{ $itemdata['tax'] }}">
        <input type="hidden" name="item_price" id="item_price_{{ $itemdata['id'] }}"
            value="{{ $price }}">
...
<script>
    ...
    function toggleCrusts(addonId, itemId) {
        let crustDiv = document.getElementById(`crust-options-${addonId}-${itemId}`);
        let addonCheckbox = document.getElementById(`addons_${addonId}_${itemId}`);

        if (addonCheckbox && crustDiv) {
            if (addonCheckbox.checked) {
                // Show crust options for the selected addon without hiding others
                crustDiv.classList.remove('d-none');
            } else {
                // Hide only the crust options related to the unchecked addon
                crustDiv.classList.add('d-none');
            }
        }
    }
</script>
