                ...
                <div class="card border-0">
                    <div class="card-body">
                        <div id="privacy-policy-three" class="privacy-policy">
                            <form method="post" action="{{ URL::to('admin/topDeals/update') }}" name="about"
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label class="form-label">Deal Products</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <select class="form-control selectpicker w-100" name="size_id[]" multiple
                                            data-live-search="true">
                                            <?php $size_ids = explode(',', $getitem->size_id); ?>
                                            @foreach (helper::get_sizes() as $item)
                                                <option value="{{ $item->id }}"
                                                    {{ in_array($item->id, $size_ids) ? 'selected' : '' }}>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <div class="dropdown bootstrap-select show-tick form-control w-100">
                                            <select class="form-control selectpicker w-100" name="product_ids[]" multiple
                                                data-live-search="true">
                                                <?php $product_ids = explode(',', $getitem->product_ids); ?>
                                                @foreach (helper::getItems() as $item)
                                                    <option value="{{ $item->id }}"
                                                        {{ in_array($item->id, $product_ids ?? []) ? 'selected' : '' }}>
                                                        {{ $item->item_name }}
                                                    </option>
                                                @endforeach
                                            </select>
