...
<ul class="navbar-nav">
    ...
    <li class="nav-item mb-2 fs-7 {{ Auth::user()->type != 1 ? (in_array('13', $modules) == true ? '' : 'd-none') : '' }}"
        id="13">
        <a class="nav-link rounded d-flex {{ request()->is('admin/deal-notifications*') ? 'active' : '' }}"
            href="{{ URL::to('/admin/deal-notifications') }}" aria-expanded="false">
            <i class="fa-solid fa-money-check-dollar-pen"></i><span
                class="nav-text ">Deal Notifications</span>
        </a>
    </li>
    @if (Auth::user()->type != 1)
