    ...
    <div class="row mt-3">
        ...
        <div class="col-12">
            <div class="card border-0 box-shadow">
                <div class="card-body">
                    <form action="{{ URL::to('admin/scripts/update/' . $script->id) }}') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control" name="name"
                                    placeholder="{{ trans('labels.name') }}" value="{{ $script->type }}" required>
                            </div>
                            <div class="col-md-6">
                                    ...
                                    <select name="branch_id" class="form-control selectpicker" required
                                        ...
                                        @foreach (helper::get_branchs() as $branch)
                                            <option value="{{ $branch->id }}"
                                                {{ $branch->id === $script->branch_id ? 'selected' : '' }}>
                                                {{ $branch->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group col-md-12">
                                <label class="form-label">{{ trans('labels.script') }}<span class="text-danger">
                                        *</span></label>
                                <textarea class="form-control" rows="5" name="script" placeholder="{{ trans('labels.script') }}"> {{ $script->script }}</textarea>
                            </div>
