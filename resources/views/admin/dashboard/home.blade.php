...
@section('content')
    ...
    <div class="container-fluid">
        @if (Auth::user()->type == 1 || in_array(0, explode(',',helper::get_roles())))
                    ...
                    <div class="card border-0 box-shadow h-100">
                        <div class="card-body">
                            <div class="d-flex align-items-center justify-content-between border-bottom pb-3 mb-3">
                                ...
                                <select class="form-select form-select-sm  w-auto" id="earningsbranch" data-url="{{ URL::to('/admin/home') }}">
                                    @forelse (helper::get_branchs() as $earnings)
                                        <option value="{{ $earnings->id }}">{{ $earnings->name }}</option>
                                    @empty
                                        <option value="" selected disabled>{{ trans('labels.select') }}</option>
                                    @endforelse
                                </select>
            ...
            <div class="row">
                @foreach($getbranchorders as $branchOrder)
                {{-- todays-orders --}}
                                        ...
                                        <tr id="dataid{{ $orderdata['id'] }}">
                                            ...
                                            <td>
                                                <div class="d-flex flex-wrap gap-1">
                                                    <a class="btn btn-sm btn-secondary square" tooltip="View" title="{{ trans('labels.view') }}" href="{{ URL::to('admin/invoice/' . $orderdata['id']) }}">
                                                        <i class="fa-regular fa-eye"></i>
                                                    </a>
                                                    <a class="btn btn-sm btn-primary square" tooltip="Print" title="{{ trans('labels.print') }}" href="{{ URL::to('admin/print/' . $orderdata['id']) }}">
                                                        <i class="fa-regular fa-print"></i>
                                                    </a>
                                                    <a href="{{ URL::to('admin/generatepdf/' . $orderdata['id']) }}" class="btn btn-warning square" tooltip="Download PDF">
