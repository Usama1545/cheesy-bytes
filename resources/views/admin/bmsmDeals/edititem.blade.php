                ...
                <div class="card border-0">
                    <div class="card-body">
                        <div id="privacy-policy-three" class="privacy-policy">
                            <form method="post" action="{{ URL::to('admin/bmsmDeals/update') }}" name="about" id="about"
                                ...
                                <div class="row">
                                        ...
                                        <div class="dropdown bootstrap-select show-tick form-control w-100">
                                            <select class="form-control selectpicker w-100" name="product_id"
                                                ...
                                                @foreach(helper::getItems() as $item)
                                                    <option
                                                            value="{{ $item->id }}" {{ $item->id === $getitem->product_id ? 'selected' : '' }}>
                                                        {{ $item->item_name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group col-md-6">
                                        ...
                                        <select class="form-control selectpicker w-100" id="bmsm_deal_type" name="bmsm_deal_type" data-live-search="true">
                                            <option value="1" {{ $getitem->bmsm_deal_type == 1 ? 'selected' : '' }}>Product Based</option>
                                            <option value="2" {{ $getitem->bmsm_deal_type == 2 ? 'selected' : '' }}>Cart Based</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group col-md-6" id="product_selector_section" style="display: none;">
                                        ...
                                        <select class="form-control selectpicker w-100" multiple name="product_ids[]"
                                            ...
                                            @foreach(helper::getItems() as $item)
                                                <option value="{{ $item->id }}" {{ in_array($item->id, $selectedDealItems) ? 'selected' : '' }}>
                                                    {{ $item->item_name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="col-sm-6 form-group" id="end_date">
                                        ...
                                        <input type="date" class="form-control" id="end_date"
                                               value="{{ $getitem->end_date }}" name="end_date"
                                               required="">
                                    </div>
                                    ...
                                    <div class="form-group col-md-12">
                                        <button class="btn btn-primary float-end" type="button" id="add_tier_button"
                                               ><span style="margin-right: 10px">Add More</span><i
                                                class="fa fa-plus"></i>
                                        </button>
                                    </div>
                                    <hr />
                                                ...
                                                @if($getitem->bmsmTiers && $getitem->bmsmTiers->count())
                                                    @foreach($getitem->bmsmTiers as $index => $tier)
                                                        <div class="tier-row row mb-2" data-index="{{ $index }}">
                                                            <div class="col-md-4">
                                                                <label>Min Qty</label>
                                                                <input type="number" name="tiers[{{ $index }}][min_qty]" class="form-control" placeholder="Min Qty / Amount" value="{{ $tier->min_qty }}" required>
                                                            </div>
                                                            <div class="col-md-3">
                                                                <label>Max Qty</label>
                                                                <input type="number" name="tiers[{{ $index }}][max_qty]" class="form-control"  placeholder="Max Qty / Amount" value="{{ $tier->max_qty }}" required>
                                                            </div>
                                                            <div class="col-md-3">
                                                                <label>Discount</label>
                                                                <input type="number" name="tiers[{{ $index }}][discount_value]" class="form-control" placeholder="Discount Value" value="{{ $tier->discount_value }}" required>
                                                            </div>
