...
class SiteController extends Controller
{
    public function branches()
    {
        ...
        $branches = Branch::with([
            ...
            ->get()
            ->map(function ($branch) use ($tz) {

                $branch->time = $branch->time->map(function ($time) use ($tz) {

                    $now = Carbon::now($tz);

                    $open = Carbon::createFromFormat('h:i A', $time->open_time, $tz)
                        ->setDate($now->year, $now->month, $now->day);

                    $close = Carbon::createFromFormat('h:i A', $time->close_time, $tz)
                        ->setDate($now->year, $now->month, $now->day);

                    if ($time->always_close == 1) {
                        $time->always_close = 1;
                    } else {
                        $time->always_close = $now->between($open, $close) ? 0 : 1;
                    }
    ...
    public function brancheTime(Request $request)
    {
        $branchId = $request->branch_id;
        $day = strtolower(\Carbon\Carbon::today()->format('l'));

        $branch = \App\Models\Branch::with([
            'time' => function ($q) use ($day) {
                $q->where('day', $day);
            }
        ])
        ->where('id', $branchId)
        ...
        $time = $branch->time->first();
        $openingHours = $time->open_time . ' till ' . $time->close_time;

        return response()->json([
            'time' => 'Images are for illustrative purposes only. Actual items may vary slightly in appearance depending on ingredients, preparation, and toppings.  Branch Time: ' . $openingHours
        ]);
    }
    ...
    public function branchChange(Request $request)
    {
        ...
        {
            $branchKey = $userId 
                ? "user_last_branch_$userId"
                : "guest_last_branch_$sessionId";
            $lastBranch = cache()->get($branchKey);
            cache()->put($branchKey, $branchId, now()->addDays(10));

            if ($lastBranch && $lastBranch != $branchId) {

                // Branch changed → clear cart
                if ($userId) {
        ...
        $response = ApiCacheHelper::remember(
            ...
            function () use ($branchId) {
                $categories = Category::select('id', 'category_name', 'slug', 'image')
                    ...
                    ->where('is_available', 1)->orderBy('reorder_id')
                    ->get();
    ...
    public function categoriesWithProducts(Request $request)
    {
        ...
        /* -----------------------------------------------------
            LEVEL 1: BASE CACHE (SHARED)
        ------------------------------------------------------ */
        $baseCacheKey = "categories_products_base_branch_{$branchId}";

        $baseData = Cache::remember($baseCacheKey, 3600, function () use ($branchId) {
            ...
            // Subcategories
            $subcategories = Subcategory::whereIn('cat_id', $categoryIds)
                ->where('is_available', 1)
                ->where('is_deleted', 2)
                ->orderBy('reorder_id')
                ->get()
            ...
            // Items
            $items = Item::with('category_info', 'subcategory_info', 'item_image')
                ->select(
                    'item.*',
                    DB::raw("MAX(CASE WHEN item_prices.branch_id = $branchId 
                        THEN COALESCE(item_prices.price, 0) 
        ...
        /* -----------------------------------------------------
            ENRICH ITEMS
        ------------------------------------------------------ */
        $products = $baseData['items']->map(function ($item) use ($favoriteIds, $cartItemIds) {
    ...
    public function ItemDetails(Request $request, $slug) 
    {
        ...
        if ($deal_category_id && empty($size_ids)) {
            $dealCategory = DealCategory::select('size_id')->find($deal_category_id);

            if ($dealCategory && $dealCategory->size_id) {
                $size_ids = [$dealCategory->size_id]; // force single size
            }
        }
        ...
        $addons_group = $addons_group->filter(function ($group) use ($addons, $is_pizza) {
            $availableAddons = $addons->where('addongroup_id', $group->id);
            
            // Convert to object with string keys to ensure consistent format
            $addonsObject = [];
            foreach ($availableAddons as $index => $addon) {
                $addonsObject[(string)($addon->id)] = [
                    'id' => $addon->id,
                    'addongroup_id' => $addon->addongroup_id,
                    'name' => $addon->name,
                    'price' => $addon->price,
                ];
            }
            
            $group->availableAddons = $addonsObject;
