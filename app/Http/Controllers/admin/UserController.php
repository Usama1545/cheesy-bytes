...
class UserController extends Controller
{
    ...
    {
        $query = User::where('type', '2')->where('is_deleted', 2)->orderBydesc('id');
        ...
        if ($request->filled('is_app_user')) {
            $is_app_user = $request->is_app_user;
            $query->where('is_app_user', $is_app_user);
        }
