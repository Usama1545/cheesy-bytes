...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Order::query()
            ...
            ->where(function ($q) {
                $q->where(function ($q) {
                    $q->where('transaction_type', 15)->where('payment_status', 2);
        ...
        ->when($branchId, fn ($q) => $q->where('branch_id', $branchId))->when($from, fn ($q) => $q->where('order_from', $from));
    ...
    public function invoice(Request $request)
    {
        ...
        $orderCustomdetails = OrderDetails::where('order_details.order_id', $request->id)
            ...
            ->with('custom_pizza.toppings', 'custom_pizza.size', 'custom_pizza.crust', 'custom_pizza.sauce', 'custom_pizza.dipping', 'deal.product')
            ->get();
