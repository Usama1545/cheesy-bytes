...
class helper
{
    ...
    {
        // Priority 1: branch from current route
        $branchSlug = request()->route('branch');

        if ($branchSlug) {
            $parameters['branch'] = $branchSlug;
            return route($name, $parameters, $absolute);
        }

        // Priority 2: branch from session
        if (session()->has('branch_id')) {
            $branch = \App\Models\Branch::find(session('branch_id'));

            if ($branch) {
                $parameters['branch'] = $branch->slug;
                return route($name, $parameters, $absolute);
            }
        }

        // Priority 3: fallback (no branch)
        return route($name, $parameters, $absolute);
    }
    ...
    public static function calculateDiscountOnApi($cart, $branchId, $user = null)
    {
        $currentDateTime = now();
            ...
            foreach ($groupedByDeal as $dealId => $items) {
                ...
                if ($deal->bmsm_deal_type == 1) {
                    ...
                    if ($bestTier) {
                        ...
                        if ($deal->offer_type == 2) {
                            $dealDiscount = ($bestTier->discount_value / 100) * $totalPrice;
                        } else {
                            $dealDiscount = $bestTier->discount_value;
                        }
            ...
            foreach ($deals as $deal) {
                ...
                if ($deal->bmsm_deal_type == 2) {
                    ...
                    if ($bestTier) {
                        ...
                        if ($deal->offer_type == 2) {
                            $discount = ($bestTier->discount_value / 100) * $totalCartPrice;
                        } else {
                            $discount = $bestTier->discount_value;
                        }
        ...
        if (
            $isFirstOrder &&
            $totalCartPrice >= ($settings->first_order_min_amount ?? 0)
        ) {
            $firstOrderDiscount = $settings->first_order_discount ?? 0;
        }

        $totalDiscount = $dealDiscount + $firstOrderDiscount;

        // Message builder
        $messageParts = [];

        if ($dealDiscount > 0) {
            $messageParts[] = "You saved $" . number_format($dealDiscount, 2) . " from deals";
        }

        if ($firstOrderDiscount > 0) {
            $messageParts[] = "On your first order you saved $" . number_format($firstOrderDiscount, 2);
        }

        if (!empty($messageParts)) {
            $discountMessage = "🎉 Hurray! " . implode(' and ', $messageParts) . ".";
        }

        return [
            'discount_amount' => round($totalDiscount, 2),
            'discount_type' => $discountMessage
        ];
    }
