...
class helper
{
    ...
    public static function image_path($image)
    {
        ...
        if (Str::contains($image, 'category')) {
            if (file_exists(public_path('admin-assets/images/category/' . $image))) {
    ...
    {
        return Category::with('item_info')->select('id', 'category_name', 'slug', 'image')->where('is_available', '=', '1')->where('is_deleted', '2')->orderBy('reorder_id')->limit($limit)->get();
    }
