...
class Item extends Model
{
    ...
    {
        $basePath = url(env('ASSETSPATHURL') . 'admin-assets/images/item/');

        return $this->hasOne('App\Models\ItemImages', 'item_id', 'id')
            ->select(
                'item_images.id',
                'item_images.item_id',
                'item_images.image as image_name',
                'item_images.thumbnail',
                DB::raw("
                    CONCAT(
                        '{$basePath}/',
                        COALESCE(item_images.thumbnail, item_images.image)
                    ) AS image_url
                ")
            );
    }
